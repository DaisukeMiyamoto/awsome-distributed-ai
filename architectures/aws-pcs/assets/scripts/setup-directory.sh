...
#
# Environment variables (env interface for manual / custom-AMI runs; the
# lifecycle action passes these positionally — see Usage above):
#   LDAP_DOMAIN_SUFFIX  — e.g. "dc=cluster,dc=internal"
#   LDAP_DOMAIN         — e.g. "cluster.internal" (derived from suffix)
#   LDAP_ADMIN_PASSWORD — generated by this script if unset (server role only)
...
EOF
    ...
    # location this script came from (S3_BUCKET/S3_KEY_PREFIX passed by the lifecycle action);
